...
export default function ViewUserPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    if (!profile) return <div className={styles.content}>User not found.</div>;
        ...
        <div className={styles.dashboardFadeIn}>
            <header style={{ marginBottom: '2rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div>
                    ...
                    <p className={styles.subtitle}>Staff User Profile & Activity</p>
                </div>
                <div className={styles.actionGroup}>
                    {canManage && (
                        <>
                            <button className={`${styles.actionBtn} ${styles.editBtn}`} onClick={() => router.push(`/dashboard/users/${id}/edit`)}>Edit User</button>
            ...
            <div className={styles.formCard}>
                <h3 className={styles.formTitle}>User Information</h3>
                <div style={{ display: 'grid', gap: '1.5rem', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))' }}>
                    ...
                    <div style={{ gridColumn: 'span 2' }}>
                        <label className={styles.formLabel}>Official Address</label>
